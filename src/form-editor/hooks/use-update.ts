...
export const useFormEffect = (effect: EffectCallback, deps: DependencyList = []) => {
...
export const useDesignEffect = (effect: EffectCallback, deps: DependencyList = []) => {
  const { mode } = useEditorContext();
  useEffect(mode !== 'design' ? () => {} : effect, deps)
};

export const useDesignUpdate = (effect: EffectCallback, deps: DependencyList) => {
  const { mode } = useEditorContext();
  const firstRender = useRef<boolean>(true);
  useEffect(firstRender.current || mode !== 'design' ? () => { firstRender.current = false } : effect, deps)
};