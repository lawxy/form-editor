...
动作相关配置和组件事件标签栏都可以进行拖拽排序，如果事件设置串联则需要调整好顺序。串联功能可用于表单提交前进行校验等功能。

<b>值发生变化</b>事件: 顾名思义，组件的表单值发生变化执行的事件。除了表格组件，<b>不要</b>同时设置关联服务更新值，而后设置值发生变化刷新同一个服务，否则会进入死循环。
...
<video width="640" height="360" controls>
  <source src="https://roddan.cn/editor-assets/getTable.mp4" type="video/mp4">
</video>

<b>分页获取</b>：表格组件如果需要分页获取数据，接口一定要返回数据<b>总量</b>并更新在组件<b>total</b>字段上，另外<b>分页切换事件</b>上传的数据是当前页码和分页条数，比如当前页 2、分页数 20，上传数据为'2,20'

<video width="640" height="360" controls>
  <source src="https://roddan.cn/editor-assets/getTable2.mp4" type="video/mp4">
</video>
